...
## view para carrito, aquí se manda la conexión captada por el json del post y luego se resta el saldo del usuario

from django.http import JsonResponse
from django.views.decorators.csrf import csrf_exempt
from .models import Plato, CartItem
...
def update_cart(request):
    if request.method == 'POST':
        ...
        try:
            plato = Plato.objects.get(nom_plato=title)
        except Plato.DoesNotExist:
            return JsonResponse({'error': 'Producto no encontrado'}, status=404)
        
        cart_item, created = CartItem.objects.get_or_create(plato=plato, user=request.user)
        cart_item.quantity = quantity
        cart_item.save()

        return JsonResponse({'message': 'Carrito actualizado', 'quantity': cart_item.quantity})
